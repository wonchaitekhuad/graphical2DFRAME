...
namespace Graphical_2D_Frame_Analysis_CSharp
{
    public partial class FSupDisp : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Math.Abs(InputParsingHelpers.ParseDoubleOrDefault(textBox1.Text.Replace(" ", ""), 0)) > 0.3)
            {
                MessageBox.Show("Support_Displacement must not exceed 0.3M !!!");
                return;
            }
            if (Math.Abs(InputParsingHelpers.ParseDoubleOrDefault(textBox2.Text.Replace(" ", ""), 0)) > 0.3)
            {
                MessageBox.Show("Support_Displacement must not exceed 0.3M !!!");
                return;
            }
